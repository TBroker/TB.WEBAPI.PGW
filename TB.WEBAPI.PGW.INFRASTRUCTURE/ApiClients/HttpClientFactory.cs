...
namespace TB.WEBAPI.PGW.INFRASTRUCTURE.ApiClients
{
    public static class HttpClientFactory
    {
        public static void RegisterHttpClients(IServiceCollection services)
        {
            ...
            services.Configure<KBankSetting>(options => { });

            // ตั้งค่า KBankExternalApi
            services.AddHttpClient("KBankExternalApi", (serviceProvider, client) =>
            {
                var apiSettings = serviceProvider.GetRequiredService<IOptions<KBankSetting>>().Value;

                client.BaseAddress = new Uri(apiSettings.ServiceUrl);
                client.DefaultRequestHeaders.Accept.Add(new MediaTypeWithQualityHeaderValue("application/json"));
                client.DefaultRequestHeaders.Add("x-api-key", apiSettings.SecretKey);
            });

            // อ่านการตั้งค่าจาก app settings
            services.Configure<T2PSetting>(options => { });          


            // ลงทะเบียน TokenHandler
            services.AddTransient<TokenHandler>();

            // ตั้งค่า T2PExternalApi
            services.AddHttpClient("T2PExternalApi", (serviceProvider, client) =>
            {
                var apiSettings = serviceProvider.GetRequiredService<IOptions<T2PSetting>>().Value;
    ...
    public class TokenHandler(IServiceProvider serviceProvider) : DelegatingHandler
    {
        ...
        {
            // ดึงข้อมูลการตั้งค่า
            var apiSettings = _serviceProvider.GetRequiredService<IOptions<T2PSetting>>().Value;
